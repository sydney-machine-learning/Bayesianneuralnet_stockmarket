...
# keras回调函数，在每个epoch结束时评估测试集性能，并且计算训练集每个step的rmse
...
for d in os.listdir('./datasets/raw'):
    ...
    history = FNN_adam.fit(train_x, train_y, epochs=1000, batch_size=32, callbacks=[fa_test_his])
    ...
    sgd_history = FNN_sgd.fit(train_x, train_y, epochs=1000, batch_size=32, callbacks=[fs_test_his])
    ...
    fs_pred_test = FNN_adam.predict(test_x)
    ...
    if steps_ahead == 1:
        plot(
            ...
            [test_y, ptmcmc_pred_test, fa_pred_test, fs_pred_test, lstm_pred_test],
            ['Actual', 'ptmcmc', 'FNN_adam', 'FNN_sgd', 'LSTM'],
        ...
        for i in [0, 1, 4]:
            plot(
                ...
                [test_y[:, i], ptmcmc_pred_test[:, i], fa_pred_test[:, i], fs_pred_test[:, i], lstm_pred_test[:, i]],
                ['Actual', 'ptmcmc', 'FNN_adam', 'FNN_sgd', 'LSTM'],
    ...
    if steps_ahead == 1:
        ptmcmc_indirmse_path = 'one_step_problemfolder/{}/indi_rmse_test.txt'.format(name)
    else:
        ptmcmc_indirmse_path = 'problemfolder/{}/indi_rmse_test.txt'.format(name)

    ptmcmc_indirmse_test = np.loadtxt(ptmcmc_indirmse_path)[-1]
    fa_indirmse_test = np.asarray(fa_test_his.test_metrics['indi_rmse_test'])[-1]
    fs_indirmse_test = np.asarray(fs_test_his.test_metrics['indi_rmse_test'])[-1]
