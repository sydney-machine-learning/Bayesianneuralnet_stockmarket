...
class ParallelTempering:
    ...
    def run_chains(self):
        ...
        return pos_w, fx_train, fx_test, rmse_train, rmse_test, indi_rmse_train, indi_rmse_test, mae_train, mae_test, mape_train, \
               mape_test, acc_train, acc_test, likelihood_vec, swap_perc, accept_vec, accept
...
def main():
    ...
    # for name in ['TWTR']:
        ...
        burn_in = 0.5  # 表示在最终生成模型预测值的时候，舍弃的训练过程的比例
                        # burn_in越大，舍弃训练初段的结果越多，理论性能越好
                        # github的源码默认为 0.5
...
##############################################################
        ...
        np.savetxt(path_db + '/ptmcmc_test_y_nor.txt', y_test, fmt='%1.6f')
        n = len(y_test)
